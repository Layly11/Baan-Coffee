...
const TableShift = ({ isFetching, rows, columns }: TableSummaryProps): JSX.Element => {
    return (
        <Container>
            <TableWrapper>
                {columns.map((value) => (
                    ...
                    Array.isArray(rows) && rows.map((row: any, rowIndex: any) => {
                        return (
                            <Row key={row.id ?? rowIndex}>
                                <DataCell>{row.employee_name !== null ? row.employee_name : '-'}</DataCell>
                                <DataCell >{row.role !== null ? row.role : '-'}</DataCell>
                                <DataCell>{row.start_time !== null && row.end_time !== null ? `${row.start_time} - ${row.end_time}` : '-'}</DataCell>
                                <DataCell >{row.status !== null ? row.status : '-'}</DataCell>
                                <DataCell>{row.note !== null ? row.note : '-'}</DataCell>
                            </Row>
                        )
                    })
                )}
